...
type RedeemCodeQuery struct {
	...
	withCreatedByReseller *UserQuery
...
// QueryCreatedByReseller chains the current query on the "created_by_reseller" edge.
func (_q *RedeemCodeQuery) QueryCreatedByReseller() *UserQuery {
	query := (&UserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(redeemcode.Table, redeemcode.FieldID, selector),
			sqlgraph.To(user.Table, user.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, redeemcode.CreatedByResellerTable, redeemcode.CreatedByResellerColumn),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
...
func (_q *RedeemCodeQuery) Clone() *RedeemCodeQuery {
	...
	return &RedeemCodeQuery{
		...
		withCreatedByReseller: _q.withCreatedByReseller.Clone(),
...
// WithCreatedByReseller tells the query-builder to eager-load the nodes that are connected to
// the "created_by_reseller" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RedeemCodeQuery) WithCreatedByReseller(opts ...func(*UserQuery)) *RedeemCodeQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withCreatedByReseller = query
	return _q
}
...
func (_q *RedeemCodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*RedeemCode, error) {
	...
	if query := _q.withCreatedByReseller; query != nil {
		if err := _q.loadCreatedByReseller(ctx, query, nodes, nil,
			func(n *RedeemCode, e *User) { n.Edges.CreatedByReseller = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RedeemCodeQuery) loadCreatedByReseller(ctx context.Context, query *UserQuery, nodes []*RedeemCode, init func(*RedeemCode), assign func(*RedeemCode, *User)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*RedeemCode)
	for i := range nodes {
		if nodes[i].CreatedByResellerID == nil {
			continue
		}
		fk := *nodes[i].CreatedByResellerID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "created_by_reseller_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *RedeemCodeQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withCreatedByReseller != nil {
			_spec.Node.AddColumnOnce(redeemcode.FieldCreatedByResellerID)
		}
	}
