...
func (s *PaymentOrderExpiryService) runOnce() {
	...
	recovered, err := s.paymentSvc.ReconcilePendingPollingOrders(reconcileCtx)
	cancel()
	if err != nil {
		slog.Warn("[PaymentOrderExpiry] failed to reconcile pending polling orders", "error", err)
	} else if recovered > 0 {
