...
type CreateGroupInput struct {
	...
	IsExclusive                bool
	SubscriptionType           string // standard/subscription/token
	TokenLimit                 int64  // legacy fixed quota, retained for compatibility
...
type UpdateGroupInput struct {
	...
	Status                     string
	SubscriptionType           string   // standard/subscription/token
	TokenLimit                 *int64   // nil = no change
	TokenPricePerMillionPerDay *float64 // nil = no change
	DailyLimitUSD              *float64 // 日限额 (USD)
	WeeklyLimitUSD             *float64 // 周限额 (USD)
	MonthlyLimitUSD            *float64 // 月限额 (USD)
	LongContextPricingEnabled  *bool
