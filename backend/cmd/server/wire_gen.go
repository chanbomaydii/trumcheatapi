...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	resellerRepository := repository.NewResellerRepository(client)
	resellerService := service.NewResellerService(resellerRepository, usageService, billingCacheService, apiKeyAuthCacheInvalidator)
	resellerHandler := handler.NewResellerHandler(resellerService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, resellerHandler, idempotencyCoordinator, idempotencyCleanupService)
