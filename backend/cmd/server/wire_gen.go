...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	publicStatusRepository := repository.NewPublicStatusRepository(db)
	publicStatusService := service.ProvidePublicStatusService(publicStatusRepository)
	publicStatusHandler := handler.NewPublicStatusHandler(publicStatusService, settingService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, publicStatusHandler, idempotencyCoordinator, idempotencyCleanupService)
