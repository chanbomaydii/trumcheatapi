  ...
  <div class="space-y-4">
    ...
    <template v-else-if="showQRCode">
      ...
      <canvas v-else ref="qrCanvas" class="mx-auto"></canvas>
            <!-- Brand logo overlay -->
      <div v-if="!isRemoteQRImage" class="pointer-events-none absolute inset-0 flex items-center justify-center">
              <span :class="['rounded-full p-2 shadow ring-2 ring-white', qrLogoBgClass]">
                <img :src="qrLogoIcon" alt="" class="h-5 w-5 brightness-0 invert" />
              </span>
            </div>
...
async function renderQR() {
  await nextTick()
  if (!showQRCode.value || isRemoteQRImage.value || !qrCanvas.value || !qrUrl.value) return
